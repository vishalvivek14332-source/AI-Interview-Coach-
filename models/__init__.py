...
def init_db(app=None) -> None:
    ...
    try:
        ...
        # Check if category column exists in interviews
        cursor = conn.cursor()
        cursor.execute("PRAGMA table_info(interviews)")
        columns = [row['name'] for row in cursor.fetchall()]
        if 'category' not in columns:
            cursor.execute("ALTER TABLE interviews ADD COLUMN category TEXT NOT NULL DEFAULT ''")
        conn.commit()
    finally:
        conn.close()
...
def create_interview(candidate: SimpleNamespace, category: str, job_role: str,
                     experience_level: str) -> SimpleNamespace:
    conn = _conn()
    try:
        cur = conn.execute(
            '''INSERT INTO interviews
               (candidate_id, category, job_role, experience_level, status, started_at)
               VALUES (?,?,?,?,?,?)''',
            (candidate.id, category, job_role, experience_level, 'in_progress', _now_iso())
        )
        conn.commit()
...
def list_interviews(search: str = '', limit: int = 50) -> list:
    ...
    try:
        ...
        result = []
        for r in rows:
            category_val = r['category'] if 'category' in r.keys() else ''
            result.append({
                'id':                  r['id'],
