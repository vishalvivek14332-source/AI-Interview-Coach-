...
====================================
Centralised AI service layer using the Anthropic Messages API directly
...
def _call_llm(prompt: str, max_tokens: int = 800, temperature: float = 0.4) -> str:
    """POST to Anthropic Messages API; return the response text."""
    if not _API_KEY:
        raise RuntimeError(
    ...
    for attempt in range(1, _MAX_RETRIES + 1):
        try:
            ...
            if resp.status_code in (400, 401, 403):
                raise RuntimeError(
                    'Anthropic API error %d: %s' % (resp.status_code, resp.text[:200])
                )
    ...
    raise RuntimeError('Anthropic API failed after %d attempts: %s' % (_MAX_RETRIES, last_exc))
...
def generate_question(
    ...
    prompt = (
        'You are a senior hiring manager interviewing a %s candidate for a %s role.\n'
        '%s\n\n'
        ...
        experience_level, job_role, context, q_type, job_role,
...
def evaluate_answer(
    ...
    prompt = (
        'You are an expert interview evaluator for %s roles.\n'
        '%s\n\n'
        'QUESTION (%s): %s\n\n'
        'CANDIDATE ANSWER: %s\n\n'
...
def generate_summary(
    ...
    prompt = (
        'You are writing a post-interview assessment for a %s role (%s level).\n'
        'Overall score: %.2f/10\n'
        ...
        job_role, experience_level, overall,
...
def health_check() -> bool:
    try:
        result = _call_llm('Reply with {"ok": true}', max_tokens=20, temperature=0)
