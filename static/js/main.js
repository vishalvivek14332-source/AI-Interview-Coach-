 ...
 */
function validateSetupForm(name, email, role, exp) {
...
async function startInterview() {
  ...
  const { valid, error } = validateSetupForm(name, email, role, exp);
  if (!valid) {
  ...
  try {
    ...
    // an extra API call.
    sessionStorage.setItem('interview_meta', JSON.stringify({ name, role, exp }));
...
function initRolePillClicks() {
  document.querySelectorAll('.role-pill').forEach((pill) => {
    pill.addEventListener('click', () => {
      // Strip the emoji prefix (e.g. "💻 Software Developer" → "Software Developer")
      const raw  = pill.textContent.trim();
      const role = raw.replace(/^\S+\s/, '');

      const select = document.getElementById('setupRole');
      if (select) {
        // Find the matching option (case-insensitive substring match).
